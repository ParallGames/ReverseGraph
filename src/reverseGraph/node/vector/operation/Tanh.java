...
public class Tanh extends VectorOperation {
	private final double[] values;
	private final double[] derivatives;

	private final VectorNode input;

	public Tanh(VectorNode input) {
		super(input.width);
	...
	public void compute() {
		for (int i = 0; i < width; i++) {
			values[i] = Math.tanh(input.get(i));
		}
	}
	...
	public void computeDependenciesDerivatives() {
		if (input instanceof VectorDerivable) {
			...
			for (int i = 0; i < width; i++) {
				derivable.addDerivative(i, derivatives[i] * (1 - values[i] * values[i]));
			}
		}
	}
