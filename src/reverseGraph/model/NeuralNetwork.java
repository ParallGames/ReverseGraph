...
public class NeuralNetwork {
	...
	public NeuralNetwork(Node inputs, LayerModel[] layerModels) {
		...
		weights[0] = Util.createNormalizedWeights(inputs.values.flat.length, layerModels[0].outputSize);
		...
		for (int i = 1; i < layers.length; i++) {
			weights[i] = Util.createNormalizedWeights(layerModels[i - 1].outputSize, layerModels[i].outputSize);
